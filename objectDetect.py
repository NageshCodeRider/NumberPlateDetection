...
from tkinter import ttk, filedialog, Label, Entry, Button
import shutil
    ...
    try:
        ...
        with mysql.connector.connect(
            ...
            for detected_number in detected_numbers:
                ...
                if student_result:
                    display_data(student_result)  # Assuming display_data function is defined
                ...
                if teacher_result:
                    display_data(teacher_result)  # Assuming display_data function is defined
...
def register():
    ...
    licence_path = licence_path_entry.get()  # Added line to get licence file path
    ...
    # Validate inputs
    if not all([reg_number, owner_name, department, contact_number, licence_path]):
    ...
    if not licence_number.endswith("IN"):
        messagebox.showerror("Error", "License number should end with 'IN'.")
        return
    ...
    try:
        ...
        insert_query = f"INSERT INTO {table_name} (reg_number, owner_name, licence_number, department, contact_number, licence_path) VALUES (%s, %s, %s, %s, %s, %s)"
        insert_data = (reg_number, owner_name, licence_number, department, contact_number, licence_path)
...
def upload_driving_license(licence_path_entry):
    try:
        file_path = filedialog.askopenfilename(initialdir="/", title="Select PDF file", filetypes=[("PDF files", "*.pdf")])
        if file_path:
            # Get the destination folder where you want to store the file
            destination_folder = "C:/Users/NAGESH/.spyder-py3/smartporject/gallery"
            
            # Copy the selected file to the destination folder
            shutil.copy(file_path, destination_folder)
            
            # Update the entry widget with the path of the copied file
            copied_file_path = os.path.join(destination_folder, os.path.basename(file_path))
            licence_path_entry.delete(0, 'end')
            licence_path_entry.insert(0, copied_file_path)
        else:
...
# Label and Entry for displaying selected file path
licence_path_label = Label(registration_window, text="Driving Licence PDF Path:")
licence_path_label.grid(row=5, column=0, padx=5, pady=5)
licence_path_entry = Entry(registration_window)  # Make the Entry widget read-only
licence_path_entry.grid(row=5, column=1, padx=5, pady=5)

# Button to upload driving license
upload_button = Button(registration_window, text="Upload License", command=lambda: upload_driving_license(licence_path_entry))
...
update_button.grid(row=7, column=1, padx=5, pady=10)
delete_button = tk.Button(registration_window, text="Delete", command=delete)
delete_button.grid(row=7, column=2, padx=5, pady=10)
